...
output_path = os.path.join(base_dir, 'daily_upcomer_links.txt')

base_url = 'https://daily.upcomer.com/overwatch/page/{}/'
...
if not os.path.exists(output_path):
    ...
    while True:
        ...
        posts = soup.find_all('h3', attrs={'class':'post__title'})
        print(len(posts))
        for p in posts:
            href = p.find('a')['href']
...
for link in links_to_scrape:
    ...
    soup = BeautifulSoup(response.content, 'html.parser')
    body = soup.find('article', attrs={'class': 'post--single'})
    if body is None:
        body = soup.find('div', attrs={'class': 'col-lg-8'})
    if body is None:
        body = soup.find('div', attrs={'class': 'col-lg-7'})
    if body is None:
        body = soup.find('div', attrs={'class': 'full-width'})
    ps = body.find_all('p')
